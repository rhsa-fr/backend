...
class Simpanan(Base):
    ...
    id_jenis_simpanan = Column(Integer, ForeignKey("jenis_simpanan.id_jenis_simpanan", ondelete="RESTRICT", onupdate="CASCADE"), nullable=False, index=True)
    ...
    tipe_transaksi = Column(Enum(TipeTransaksi), nullable=False, index=True)
    nominal = Column(DECIMAL(15, 2), nullable=False)
    saldo_akhir = Column(DECIMAL(15, 2), nullable=False, default=0)
    keterangan = Column(Text)
    id_user = Column(Integer, ForeignKey("user.id_user", ondelete="SET NULL", onupdate="CASCADE"))
